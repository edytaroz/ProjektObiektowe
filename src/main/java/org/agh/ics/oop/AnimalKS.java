...
public class AnimalKS extends AnimalSzalony {
    public AnimalKS(int energyLoss, int energy, int childEnergy, int lenOfGenome, int plantEnergy, int satietyLevel,
    ...
    public List<Animal> conflictReproduction(List<Animal> animals) { //assumes there are at least 2 animals
        ...
        for (int i = 0; i < possibleParents.size() / 2; i++) {
            Animal child = new AnimalKS(possibleParents.get(2 * i), possibleParents.get(2 * i + 1));
            children.add(child);
        }
